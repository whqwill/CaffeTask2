...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  train_db->Open(output_folder + "/cifar100_train_seed23_" + db_type, db::NEW); 
  scoped_ptr<db::DB> validate_db(db::GetDB(db_type));
  validate_db->Open(output_folder + "/cifar100_validate_seed23_" + db_type, db::NEW); 
  int num_train[100] = {0};
  ...
  for (int itemid = 0; itemid < kCIFARTrainSize; ++itemid) {
	...
	num_train[label]++;
	std::cout << itemid << "belongs to training set" << std::endl;
	sum_train++;
    }
    else {
	txn_validate->Put(string(str_buffer, length), out);
	num_validate[label]++;
	std::cout << itemid << "belongs to validation set" << std::endl;
	sum_validate++;
    }
  ...
  train_db->Close();
  validate_db->Close();
  std::cout << "train_db and validate_db has been closed." << std::endl;
/*
