...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  train_db->Open(output_folder + "/cifar100_train_cluster_" + db_type, db::NEW);
  scoped_ptr<db::DB> validate_db(db::GetDB(db_type));
  validate_db->Open(output_folder + "/cifar100_validate_cluster_" + db_type, db::NEW); 
  int num_train[100] = {0};
  ...
  scoped_ptr<db::Transaction> txn_train(train_db->NewTransaction());
  ...
  for (int itemid = 0; itemid < kCIFARTrainSize; ++itemid) {
    ...
    int newlabel = cluster_label[label][0]-1;
    datum.set_label(newlabel);
	...
	std::cout << itemid << " belongs to training set " << label << " " << newlabel << std::endl;
        txn_train->Put(string(str_buffer, length), out); 
	num_train[label]++;
	sum_train++;
    }
    else {
	std::cout << itemid << " belongs to validation set " << label << " " << newlabel << std::endl;
	txn_validate->Put(string(str_buffer, length), out);
	num_validate[label]++;
	sum_validate++;
    }
  ...
  txn_train->Commit();
  train_db->Close();
  std::cout << "train_db " << " has been closed." << std::endl;
  txn_validate->Commit();
  validate_db->Close();
  std::cout << "validate_db " << " has been closed." << std::endl;
