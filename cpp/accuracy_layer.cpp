...
void AccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int k = 0; k < top_k_; k++) {
      if (k == 0) {
	conM[bottom_data_vector[k].second][static_cast<int>(bottom_label[i])] += 1;
	std::cout << bottom_data_vector[k].second << " " << static_cast<int>(bottom_label[i]) << std::endl;	
      }
