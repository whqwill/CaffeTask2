...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  std::cout << (input_folder + "/validate.clustering.label").c_str() << std::endl;
  std::ifstream file_cluster((input_folder + "/validate.clustering.label").c_str(),std::ios::in);
  ...
  for (int i = 0; i < cluster_num; i++) {
	...
	ss << i+1 << "(" << num_each_cluster[i+1] << ")";
  	(*test_db[i])->Open(output_folder + "/cifar100_test_" + db_type + ss.str(), db::NEW); 
  } 
  ...
  std::ifstream file_cluster_test((input_folder + "/test.clustering.label").c_str(),std::ios::in);
