...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int i = 0; i < cluster_num; i++) {
	...
	ss << i+1 << "(" << num_each_cluster[i+1] << ")";
  	(*test_db[i])->Open(output_folder + "/cifar100_test_divide_from_training_cluster_iter10000" + db_type + ss.str(), db::NEW); 
  } 
  ...
  std::ifstream file_cluster_test((input_folder + "/test.cluster.label.iter10000").c_str(),std::ios::in);
